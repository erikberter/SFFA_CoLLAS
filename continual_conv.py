from ff_mod.networks_cl.network.ff_network import FF_Network, FF_Layer, FF_Network_MultiTask, FF_Network_Incremental, FF_Layer_Conv
from ff_mod.overlay import AppendToEndOverlay, ImageOverlay
...
class MLP(nn.Module, BaseModel):
    def __init__(self, input_size=28 * 28, hidden_size=256, hidden_layers=2,
        ...
        layers = nn.Sequential(*(nn.Conv2d(input_size, 32, kernel_size=7, stride=2),
...
def get_ff_model(type = None, seed = 0):
    if seed <= 20:
        overlay_function = ImageOverlay(10, 28,28,1, disp=0.35, min=0.035)
    else:
        overlay_function = ImageOverlay(10, 32,32,3, disp=0.35, min=0.035)
    #overlay_function = AppendToEndOverlay(100, device="cuda:0")

    if seed <= 20:
        layer_1 = FF_Layer_Conv(1, 64, bias=True)
        layer_2 = FF_Layer_Conv(64, 128, bias=True)
    else:
        layer_1 = FF_Layer_Conv(3, 64, bias=True)
        layer_2 = FF_Layer_Conv(64, 128, bias=True)

    #layer_1 = FF_Layer(32*32*3+10, 1000, bias=True)
    #layer_2 = FF_Layer(1000, 1000, bias=True)
    
    if type == "ci":
        model = FF_Network_Incremental(2, False, True, is_conv=True)
    elif type == "di":
        model = FF_Network(False, True, is_conv=True)
        model.total_classes = 2
    else:
        model = FF_Network_MultiTask(False, True, is_conv=True)
...
def get_sd_model(type, seed=0):
    if seed <= 20:
        if type == "ci":
            model = MLP(hidden_size=288, hidden_layers=2, initial_out_features=2, input_size=1)
        elif type == "di": # Domain Incremental
            model = MLP(hidden_size=288, hidden_layers=2, output_size=2, input_size=1)
        else:
            model = MultiHeadMLP(hidden_size=288, hidden_layers=2, input_size=1 )
    else:
        if type == "ci":
            model = MLP(hidden_size=512, hidden_layers=2, initial_out_features=2, input_size=3)
        elif type == "di":
            model = MLP(hidden_size=512, hidden_layers=2, output_size=2, input_size=3)
        else:
            model = MultiHeadMLP(hidden_size=512, hidden_layers=2, input_size=3)
    model.cuda()
...
def test(model, optmizer, criterion, plugin, type : str = 'ci', seed : int = 0, name="test"):
    ...
    eval_plugin = EvaluationPlugin(
        ...
        loggers=[TensorboardLogger("experiments/tensorboard/conv_test_" + str(seed) + "_" + name)]
    )
    ...
    for task_type in ['ci', 'di', 'ti']:
        ...
        #for model_type in ['ff']:
            ...
            for plugin in ["none", "mas", "rep", "ewc", "syn", "gem"]:
                ...
                final_results[f'{task_type}_{model_type}_{plugin_str}'] = test(model, optimizer, criterion, plugin, task_type, seed, name=f'conv_{task_type}_{model_type}_{plugin_str}')