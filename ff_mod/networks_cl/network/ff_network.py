...
class FF_Layer_Conv(nn.Conv2d):
    ...
    def __init__(
        ...
        super().__init__(in_features, out_features, kernel, stride=stride, bias=bias, device=device, dtype=dtype)
    ...
    def forward(self, x, mask = None):
        ...
        pre_act = super().forward(x)
        result = self.activation(pre_act.clone()).clone()

        """if mask is not None:
            result = result * mask
        
        # Set all but the top k to zero
        # Result shape is [Batch, Features, Height, Width]
        val_ex, ind_ex = torch.topk(result[:,:mid].flatten(start_dim = 1), 15, dim=1, sorted=True)
        val_in, ind_in = torch.topk(result[:,mid:].flatten(start_dim = 1), 15, dim=1, sorted=True)
                
        mask_ex = result[:,:mid] >= val_ex[:, -1].unsqueeze(1).unsqueeze(2).unsqueeze(3)
        mask_in = result[:,mid:] >= val_in[:, -1].unsqueeze(1).unsqueeze(2).unsqueeze(3)
        
        
        result[:,:mid] = result[:,:mid] * mask_ex.float()
        result[:,mid:] = result[:,mid:] * mask_in.float()
        """
...
class FF_Network_Incremental(DynamicModule):
    ...
    def goodness(self, x):
        ...
        #goodness = F.sigmoid(x.pow(2).mean(1) - 0.7)
           
        #print(f"Mean X [{x.shape[1]}]: {x.mean()}")
        #print(f"Mean goodness [{x.shape[1]}]: {goodness.mean()}")
        # Done to avoid divisions by zero or problems in loss
        goodness = torch.clamp(goodness, 0.0001, 0.9999)
